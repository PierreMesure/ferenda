...
- **No source base class, no inheritance for sources.** There is deliberately no
  ...
  harvest engine driven by a per-agency data registry beats 82 bespoke
...
drives one shared harvest engine for 82 agency scopes over 77
författningssamlingar from a data registry (`foreskrift/agencies.py`) rather
than 82 bespoke pipelines — that is the model to follow when sources are
similar.
